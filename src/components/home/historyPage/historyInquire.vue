        ...
        <el-col :span="8">
          <el-button-group>
            <el-tooltip content="表格" placement="bottom" effect="light">
              <el-button type="primary" icon="el-icon-s-grid" @click="dataListBtn" :class="isTable?'tableIcon':'lineIcon'"></el-button>
            </el-tooltip>
            <el-tooltip content="折线图" placement="bottom" effect="light">
              <el-button type="primary" icon="el-icon-s-marketing" @click="dataChartBtn" :class="isTable?'lineIcon':'tableIcon'"></el-button>
            </el-tooltip>
          </el-button-group>
        </el-col>
        <el-col :span="5">
        ...
        <el-col :span="6" v-show="pageControlBtnShow">
          <el-button-group>
            <el-tooltip content="上一页" placement="bottom" effect="light">
              <el-button type="primary" icon="el-icon-arrow-left" @click="pageUpBtn" :disabled="this.loadingFlag"></el-button>
            </el-tooltip>
            <el-tooltip content="下一页" placement="bottom" effect="light">
              <el-button type="primary" icon="el-icon-arrow-right" @click="pageDownBtn" :disabled="this.loadingFlag"></el-button>
            </el-tooltip>
          </el-button-group>
...
export default {
  data () {
    return {
      ...
      isTable: true,
      // ...导出表格所用变量
      downloadBtnFlag: false,       // 导出表格事件标志位
      onenetPriDataAll: [],
      limitDataAll: [],
      monitorPointDataAll: []
    }
  ...
  methods: {
    ...
    queryHistoryBt () {
      ...
      // this.nodeSelected = 'FDFD0001'
      // test3 ... 4页数据 实际查到了 4000+4000+4000+1372()
      this.valueStart = '2019-04-28T00:00:00'
      this.valueEnd = '2019-04-30T14:00:00'
      this.nodeSelected = 'FDFD0001'
      ...
      // 是否下载表格事件
      if (!this.downloadBtnFlag) {
        // ...非下载
        this.onenetOneHistoryQuery()
      } else {
        // ...下载表格，一次性查询所有数据
        this.onenetOneHistoryQueryAll()
      }
    },
    ...
    pageUpBtn: function () {
      ...
      // ...判断上一页是back 还是 cache
      let pageNextCompare = this.pageRightNum + this.pageLeftNum - 5 // 因右边已经定位好了，所以右边 - '5'即可
    ...
    onenetNextHistoryQuery: function (cursor) {
      ...
      onenet.onenetStartNextHisQue(this.param).then(function (response) {
        ...
        if (this.pageDownOrUp)  {
          ...
          this.pageRightNum++ // 上一页则pageBackNum-1
        } else {
          // ....上一页
          this.pageLeftNum-- // 上一页则pageBackNum-1
        }
        // streamId正序排序
        this.sortByKey(response.data.datastreams, 'id')
        // 筛选出4路数据中最小长度，保存到this.limitData
        ...
        console.log(this.onenetPriData.datastreams[0].datapoints.length)
        for (let i = 0; i < this.onenetPriData.datastreams.length; i++) {
          if (this.onenetPriData.datastreams[i].datapoints.length < 4001) { // 4001 一定要注意啊 最早是5000
    ...
    */
    // onenet查询近期最多limit个历史数据
    onenetOneHistoryQueryAll: function () {
      this.onenetPriDataAll = {}
      this.monitorPointDataAll = []
      onenet.onenetStartOneHisQue(this.param).then(function (response) {
        console.log('onenet查询近期最多limit个历史数据')
        console.log(response)
        console.log(2)
        ...
        let dataMinLength = Math.min.apply(Math, lengthData) // onenet数据4路数组中最小长度
        // back新数据保存到onenetPriData
        this.onenetPriDataAll = response.data // response.data引用类型所以是sort排序之后的
        console.log(this.onenetPriDataAll)
        // 遍历onenetPriData，转化格式存储到limitData
        let data = []
        if (this.onenetPriDataAll.datastreams.length > 0) {
          for (let i = 0; i < dataMinLength; i++) {
            let obj = {}
            obj.number = i + 1
            obj.time = this.onenetPriDataAll.datastreams[0].datapoints[i].at.slice(0, -4)
            obj.CH1 = this.onenetPriDataAll.datastreams[0].datapoints[i].value
            obj.CH2 = this.onenetPriDataAll.datastreams[1].datapoints[i].value
            obj.CH3 = this.onenetPriDataAll.datastreams[2].datapoints[i].value
            obj.CH4 = this.onenetPriDataAll.datastreams[3].datapoints[i].value
            data[i] = obj
          }
        }
        this.limitDataAll = data
        // 只查询一次进行this.limitData迁移到表格MointerPointerData中展示
        // * 目的是为了调用之前的程序，因为之前程序是基于limitData的
        ...
        if (response.data.cursor !== undefined) {
          this.onenetNextHistoryQueryAll(response.data.cursor)
        } else {
          this.loadingFlag = false
          this.openDownloadDialog()
        }
    ...
    // onenet查询近期最多limit个历史数据,从cursor开始
    onenetNextHistoryQueryAll: function (cursor) {
      this.param.cursor = cursor
      onenet.onenetStartNextHisQue(this.param).then(function (response) {
        console.log('onenet查询近期最多limit个历史数据,从cursor开始')
        console.log(response)
        ...
        this.onenetPriDataAll.count = this.onenetPriDataAll.count + response.data.count
        // streamId正序排序
        this.sortByKey(response.data.datastreams, 'id')
        // 筛选出4路数据中最小长度，保存到this.limitData
        let lengthData = [] // 4路传感器数组的各个长度拼成一个新辅助数组
        ...
        let dataMinLength = Math.min.apply(Math, lengthData) // onenet数据4路数组中最小长度
        // back新数据保存到onenetPriData
        for (let i = 0; i < this.onenetPriDataAll.datastreams.length; i++) {
          for (let j = 0; j < dataMinLength; j++) {
            this.onenetPriDataAll.datastreams[i].datapoints.push(response.data.datastreams[i].datapoints[j])
          }
        }
        // 遍历onenetPriData，转化格式存储到limitData
        let data = []
        if (this.onenetPriDataAll.datastreams.length > 0) {
          // 上边已经确保最新Back数据每路数据量相同，从而确保this.onenetPriDataAll.datastreams的每路数据量相同，所以可以固定写datastreams[0]
          for (let i = 0; i < this.onenetPriDataAll.datastreams[0].datapoints.length; i++) {
            let obj = {}
            obj.number = i + 1
            obj.time = this.onenetPriDataAll.datastreams[0].datapoints[i].at.slice(0, -4)
            obj.CH1 = this.onenetPriDataAll.datastreams[0].datapoints[i].value
    ...
    fillSomeDataAll: function () {
      console.log(1234)
      console.log(this.limitDataAll)
      if (this.limitDataAll.length === 0) return
      // 日期格式转换，统一为"yyyy-MM-dd HH:mm:ss"格式，以便后续进行比较
      var valueStart = this.valueStart.replace(/T/, ' ')
      var valueEnd = this.valueEnd.replace(/T/, ' ')
      if (this.monitorCreatetime > valueEnd) {
        ...
        if (this.limitDataAll[0].time > valueEnd) {
          // this.monitorPointDataAll = [] // 源程序不需要这行？
          this.limitDataAll = []
        } else {
          if (this.limitDataAll[0].time >= valueStart) {
            this.limitDataAll = this.limitDataAll.filter(function (item, index, array) {
              return (item.time <= valueEnd)
            })
          } else if (this.limitDataAll[0].time < valueStart) {
            this.limitDataAll = this.limitDataAll.filter(function (item, index, array) {
              return ((item.time >= valueStart) && (item.time <= valueEnd))
            })
          }
      ...
      // // limitDataAll -> monitorPointDataAll
      // for (let i = 0; i < this.limitDataAll.length; i++) {
      //   this.monitorPointDataAll.push(this.limitDataAll[i])
      // }
      this.monitorPointDataAll = this.limitDataAll
      console.log(this.monitorPointDataAll)
    },
    // 存储折线图数据,绘制图表
    ...
    */
    ...
    openDownloadDialog () {
      ...
      require.ensure([], () => {
        ...
        var list = this.monitorPointDataAll
        var data = this.formatJson(filterVal, list)
        exportJsonToExcel(tHeader, data, '传感器数据')
        console.log(5)
        this.limitDataAll = null
        this.monitorPointDataAll = null
      })
    },
    // excel表格数据格式处理
