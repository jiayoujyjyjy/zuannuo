...
export default {
  ...
  methods: {
    ...
    pageUpBtn: function () {
      ...
      console.log(this.cursorArr)
      console.log(this.pageCurNum)
      console.log(this.pageDataNum)
      console.log(this.pageDownBackNum)
      console.log(this.pageUpBackNum)
      // 点击上一页 页码就加1
      this.pageCurNum -= 1
      this.pageDataNum -= 1
      // 综合判断
    ...
    */
    ...
    onenetNextHistoryQuery: function (cursor) {
      ...
      onenet.onenetStartNextHisQue(this.param).then(function (response) {
        ...
        // 如果是下一页查询则push新cursor到cursorArr数组中去，上一页则不操作
        ...
        this.onenetPriData.cursor = response.data.cursor
        // // 如果是下一页查询则push新cursor到cursorArr数组中去，上一页则不操作
        // if (this.pageDownOrUp && response.data.cursor !== undefined && this.pageCurNum > this.pageDownBackNum) {
        //   this.cursorArr.push(response.data.cursor)
        // }
        ...
        for (let i = 0; i < this.onenetPriData.datastreams.length; i++) {
          if (this.onenetPriData.datastreams[i].datapoints.length < 5000) {
            ...
            if (this.pageDownOrUp) {
              ...
              // ...去尾填头 unshift方法要先插入最后一个数据，逆序插入
              console.log('去尾填头')
              console.log(this.onenetPriData.datastreams[0].datapoints)
        ...
        // // ...test
        // console.log(this.onenetPriData.datastreams[0].datapoints)
        // if (this.onenetPriData.datastreams[0].datapoints.length < 2000) {
        //   console.log('小于2000')
        //   for (let j = 0; j < dataMinLength; j++) {
        //     this.onenetPriData.datastreams[0].datapoints.push(response.data.datastreams[0].datapoints[j])
        //   }
        // } else {
        //   console.log('大于等于2000')
        //   this.onenetPriData.datastreams[0].datapoints.splice(0, 1000)
        //   console.log(this.onenetPriData.datastreams[0].datapoints)
        //   for (let j = 0; j < dataMinLength; j++) {
        //     this.onenetPriData.datastreams[0].datapoints.push(response.data.datastreams[0].datapoints[j])
        //   }
    ...
    fillSomeData: function () {
      ...
      if (this.monitorCreatetime > valueEnd) {
        ...
        // limitData: 所请求之后的并且筛选时间范围之后的所有的数据
        // monitorPointData: 大致等同于limitData，表格显示绑定数据
