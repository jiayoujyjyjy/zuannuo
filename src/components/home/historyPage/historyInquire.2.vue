...
export default {
  data () {
    return {
      ...
      // ...几个数据存储变量说明
      // * onenetPriData--onenet最新原始数据(排序后) 
      ...
      // ...分页用变量
      page: 0,           // 第一条数据
      pageSizeNumber: 1000,      // 第1000条数据
      numberConst: 1000, // number常量 后期仅需修改numberConst即可动态设置每页数据
      // test
      pageBackNumber: 0, // 只增加不减少，表示最新数据页面量，===请求back次数
      pageDataNumber: 0, // 0，1，2，3，4 表示5个元素数组的索引号，不会大于5的，
  ...
  methods: {
    ...
    pageDownBtn: function () {
      ...
      this.pageSizeNumber += this.numberConst
      // 判断下一页是否可以调用已缓存请求数据
      if (this.page * this.numberConst < this.limitData.length) {
        this.loadingFlag = true
        this.fillSomeData()
        return
      }
      // onenetPriData存储每一次请求最新返回的数据
      console.log(this.onenetPriData.cursor)
      console.log(this.endTimeFlag)
      if (this.onenetPriData.cursor !== undefined && !this.endTimeFlag) {
        this.loadingFlag = true
        this.onenetNextHistoryQuery(this.onenetPriData.cursor)
      } else {
    ...
    */
    ...
    fillSomeData: function () {
      ...
      if (this.monitorCreatetime > valueEnd) {
        ...
        let data = this.limitData.slice(this.page * this.numberConst, this.pageSizeNumber)
